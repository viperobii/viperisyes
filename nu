...
-- Dropdown to select bring speed
Setting:AddDropdown({
    Name = "Bring Mob Mode",
    Flag = "BringMobModeDropdown",
    Description = "Select Bring Mob Speed Mode",
...
-- Toggle to enable or disable bringing mobs
Setting:AddToggle({
    Name = "Bring Mobs",
    Flag = "BringMobsToggle",
    Description = "Toggles whether to bring mobs to the player.",
    Default = false,
    Callback = function(Value)
...
-- Loop that brings mobs based on selected distance
spawn(function()
    while task.wait() do
        pcall(function()
            if _G.BringMonster and _G.Level and StartMagnet and Mon and PosMon then
                for _, mob in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                    if mob.Name == Mon and
                       mob:FindFirstChild("Humanoid") and
                       mob:FindFirstChild("HumanoidRootPart") and
                       mob.Humanoid.Health > 0 and
                       (mob.HumanoidRootPart.Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= (_G.BringModeDistance or 250) then
...
Setting:AddToggle({
    Name = "Remove Damage",
    Flag = "Setting/RemoveDamage",
    Default = true,
    Callback = function(Value)
        if Value then
            for _, v953 in pairs(game.Workspace:GetDescendants()) do
                if v953.Name == "DamageCounter" then
                    v953:Destroy();
                end
            end
            for _, v955 in pairs(game.ReplicatedStorage:GetDescendants()) do
...
Trial:AddToggle({
    ...
    Description = "<font color='rgb(0, 255, 0)'>Farm Bones & V4 Active</font>",
    Default = false,
    Callback = function(v)
        _G.BoneFarmV4 = v
