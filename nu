...
Title = "Tab Prehistoric Island"

}),

Mirage = v15:AddTab({

Title = "Mirage Island & Moon"

}),

Kitsune = v15:AddTab({

Title = "Tab Kitsune Island"

}),

Draco = v15:AddTab({

Title = "Tab Draco Race"
...
task.spawn(function()
    while wait() do
        pcall(function()
            ...
            elseif (ChooseWeapon == "Blox Fruits") then -- Changed from "Blox Fruit" to "Blox Fruits"
...
Tween(CFrame.new(v839.HumanoidRootPart.Position + Vector3.new(0, 20, 0)));
...
local v499 = v16.Kitsune:AddParagraph({
...
local v500 = v16.Kitsune:AddToggle("ToggleEspKitsune", {
...
local v501 = v16.Kitsune:AddToggle("ToggleTPKitsune", {
...
local v502 = v16.Kitsune:AddToggle("ToggleCollectAzure", {
...
local v514 = v16.Sea:AddToggle("AutoDriveBoat", {
    Title = "Auto Drive Boat",
    Description = "Use Boat to Automatically Drive",
    Default = false
});

v17.AutoDriveBoat:SetValue(false);

v514:OnChanged(function(v595)
    _G.AutoDriveBoat = v595;
end);

local v511 = {}; -- Store available boats
local v512 = false; -- Lock for finding boat
local v513 = false; -- Notification lock
local FlyHeight = 0; -- Medium fly height (adjust as needed)
local BoatSpeed = 350; -- Boat speed

v505.RenderStepped:Connect(function()
    if not _G.AutoDriveBoat then
        v513 = false;
        return;
    end
    ...
    -- Function to find and get on a boat if not already on one
    local function v598()
        if v512 then
            return;
        end
    ...
    local v600 = false; -- Flag if player is on boat
    local v601 = nil; -- Current boat seat
    
    -- Check if player is on a boat or find available boats
    ...
    -- Set boat speed and make it fly at medium height
    v601.MaxSpeed = BoatSpeed;
    
    -- Make the boat fly at medium height
    local targetY = v601.Position.Y
    if v601.Position.Y < FlyHeight then
        targetY = FlyHeight
    end
    
    v601.CFrame = CFrame.new(Vector3.new(v601.Position.X, targetY, v601.Position.Z)) * v601.CFrame.Rotation;
    
    -- Auto drive forward
    v506:SendKeyEvent(true, "W", false, game);
    
    -- Make boat parts non-collidable for smoother movement
    ...
    -- Check for sea threats (seabeasts, sharks, piranhas, etc.)
    local nearbyThreat = false
    local threatDetectionRange = 300 -- Detection range
    local threatName = ""
    
    -- Check specifically for SeaBeasts in the SeaBeasts folder
    local seaBeasts = v507:FindFirstChild("SeaBeasts")
    if seaBeasts then
        for _, entity in pairs(seaBeasts:GetChildren()) do
            if entity.Name:find("SeaBeast") then
                local distance = (entity:GetPivot().Position - v597:GetPivot().Position).Magnitude
                if distance < threatDetectionRange then
                    nearbyThreat = true
                    threatName = entity.Name
                    break
                end
            end
        end
    ...
    -- Check for specific enemies in the Enemies folder as provided
    if not nearbyThreat then
        local enemies = v507:FindFirstChild("Enemies")
        if enemies then
            -- Check for specific sea threats with exact names
            local specificThreats = {
                "Terrorshark",
                "Piranha", 
                "Shark",
                "Fish Crew Member"
            ...
            for _, threatType in ipairs(specificThreats) do
                local entity = enemies:FindFirstChild(threatType)
                if entity then
                    local distance = (entity:GetPivot().Position - v597:GetPivot().Position).Magnitude
                    if distance < threatDetectionRange then
                        nearbyThreat = true
            ...
            -- Check for pirate ships/brigades
            if not nearbyThreat then
                local pirateShips = {
                    "PirateGrandBrigade",
                    "PirateBrigade"
                ...
                for _, enemy in pairs(enemies:GetChildren()) do
                    for _, shipType in ipairs(pirateShips) do
                        if enemy.Name == shipType then
                            local distance = (enemy:GetPivot().Position - v597:GetPivot().Position).Magnitude
                            if distance < threatDetectionRange then
                                nearbyThreat = true
                                threatName = enemy.Name
                                break
                            end
                        end
                    end
    ...
    -- Stop boat if threat detected
    if nearbyThreat then
        -- Stop pressing W key
        v506:SendKeyEvent(false, "W", false, game);
        
        -- Make character jump
        v506:SendKeyEvent(true, "Space", false, game);
        wait(0.1)
        v506:SendKeyEvent(false, "Space", false, game);
        
        if not v513 then
            v14:Notify({
                Title = "Viper Hub Notify",
                Content = "Stopped boat - Detected: " .. threatName,
                Duration = 5
            });
            v513 = true;
            wait(5) -- Reset notification lock after 5 seconds
            v513 = false;
        end
    else
        v513 = false;  -- Reset notification lock if no threats
    end
end);

local v510 = v16.Volcano:AddToggle("AutoFindPrehistoric", {
...
local v514 = v16.Mirage:AddToggle("AutoFindMirage", {
...
local v517 = v16.Levi:AddToggle("AutoComeHydra", {
...
local AutoRemoveRock = v16.Sea:AddToggle("ToggleAutoRemoveRock", { 
    Title = "Auto Remove Sea Rocks", 
    Description = "Best For Driving Boat No Disturbing Rocks", 
    Default = false 
})

AutoRemoveRock:OnChanged(function()
    local enabled = AutoRemoveRock.Value
    
    if enabled then
        print("Auto Remove Sea Rocks enabled")
        task.spawn(function()
            while AutoRemoveRock.Value do
        ...
        print("Auto Remove Sea Rocks disabled")
    end
end)

v16.Sea:AddButton({

Title = "Fly To BoatDealer Tiki Outpost",
...
local v522 = v16.Event:AddToggle("ToggleTerrorshark", {
    ...
    while wait() do
        if _G.AutoTerrorshark then
            pcall(function()
                ...
                if humanoid then
                    -- Consider low health if below 30% of max health
                ...
                if game:GetService("Workspace").Enemies:FindFirstChild("Terrorshark") then
                    for _, v1445 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        if v1445.Name == "Terrorshark" and v1445:FindFirstChild("Humanoid") and v1445:FindFirstChild("HumanoidRootPart") and v1445.Humanoid.Health > 0 then
                            ...
                            -- Function to detect Terrorshark attacks (by animation or other indicators)
                            local function isTerrosharkAttacking()
                                -- Check if Terrorshark is playing attack animation
                                ...
                                -- Additionally check for projectiles or special parts that might indicate attacks
                            ...
                            repeat
                                ...
                                if humanoid then
                                    if lowHealth then
                                        -- Check if player has recovered enough health (5000 health in Blox Fruit)
                                        if humanoid.Health >= 5000 or (tick() - healthRecoveryStartTime > 15) then
                                            lowHealth = false
                                        else
                                            -- Stay at safe height until health recovers
                                ...
                                -- Auto equip melee only (continuously check to ensure weapon is equipped)
                                ...
                                -- Ensure Terrorshark properties are set correctly
                                if v1445 and v1445:FindFirstChild("HumanoidRootPart") and v1445:FindFirstChild("Humanoid") then
                                    ...
                                    -- Detect if Terrorshark is attacking to dodge
                                    attackDetected = isTerrosharkAttacking()
                                    
                                    -- Handle movement logic
                                    local currentTime = tick()
                                    
                                    -- Dodge logic (takes priority)
                                    if attackDetected and not flyingUp then
                                        flyingUp = true
                                        Tween(v1445.HumanoidRootPart.CFrame * CFrame.new(0, 170, 0)) -- Dodge to 170 studs
                                        wait(1) -- Stay up for 1 second to avoid attack
                                        flyingUp = false
                                        attackDetected = false
                                        
                                        if v1445 and v1445:FindFirstChild("HumanoidRootPart") then
                                            Tween(v1445.HumanoidRootPart.CFrame * CFrame.new(0, normalHeight, 0)) -- Return to normal height
                                        end
                                    -- Regular movement pattern (every 3 seconds)
...
local v523 = v16.Event:AddToggle("TogglePiranha", {
...
local v524 = v16.Event:AddToggle("ToggleShark", {
...
local v525 = v16.Event:AddToggle("ToggleFishCrew", {
...
local v526 = v16.Event:AddToggle("ToggleShip", {
...
local v527 = v16.Event:AddToggle("ToggleGhostShip", {
...
local v531 = v16.Mirage:AddSection("Mysterious Island");

local v532 = v16.Mirage:AddParagraph({
...
local v66 = v16.Mirage:AddToggle("ToggleTpAdvanced", {
...
local v67 = v16.Mirage:AddToggle("ToggleTweenGear", {
...
local v68 = v16.Mirage:AddToggle("Togglelockmoon", {
        ...
        if _G.Auto_SkullGuitar then
            pcall(function()
                ...
                if not GetWeaponInventory("Skull Guitar") then
                    local targetPos = Vector3.new(-9681.458, 6.14, 6341.372)
                    if (targetPos - rootPart.Position).Magnitude <= 5000 then
                        ...
                        elseif hauntedCastle.Candle1.Transparency == 0 then
                            if hauntedCastle.Placard1.Left.Part.Transparency == 0 then
                                -- Solve puzzle sequence
                                local placards = {
                                    hauntedCastle.Placard7.Left.ClickDetector,
                                    hauntedCastle.Placard6.Left.ClickDetector,
                                    hauntedCastle.Placard5.Left.ClickDetector,
                                    hauntedCastle.Placard4.Right.ClickDetector,
                                    hauntedCastle.Placard3.Left.ClickDetector,
                                    hauntedCastle.Placard2.Right.ClickDetector,
                                    hauntedCastle.Placard1.Right.ClickDetector
                                }
                                Tween(CFrame.new(-8762.691, 176.847, 6171.307))
                                repeat wait() until (rootPart.Position - Vector3.new(-8762.691, 176.847, 6171.307)).Magnitude <= 3 or not _G.Auto_SkullGuitar
                                for _, clickDetector in ipairs(placards) do
                                    wait(1)
                                    fireclickdetector(clickDetector)
                                end
                            elseif hauntedCastle.Tablet.Segment1:FindFirstChild("ClickDetector") then
                                -- Solve lab puzzle
                                local puzzle = hauntedCastle["Lab Puzzle"].ColorFloor.Model
                                local parts = {puzzle.Part3, puzzle.Part4, puzzle.Part6, puzzle.Part8, puzzle.Part10}
                                Tween(CFrame.new(-9553.598, 65.623, 6041.588))
                                repeat wait() until (rootPart.Position - Vector3.new(-9553.598, 65.623, 6041.588)).Magnitude <= 3 or not _G.Auto_SkullGuitar
                                ...
                                for _, enemy in pairs(workspace.Enemies:GetChildren()) do
                                    if enemy:IsA("Model") and enemy.Name == "Living Zombie" and enemy:FindFirstChild("HumanoidRootPart") then
                                        EquipTool(SelectWeapon)
                                        enemy.HumanoidRootPart.Size = Vector3.new(60, 60, 60)
                                        enemy.HumanoidRootPart.Transparency = 1
                                        enemy.Humanoid.JumpPower = 0
                                        enemy.Humanoid.WalkSpeed = 0
                                        enemy.HumanoidRootPart.CanCollide = false
                                        enemy.HumanoidRootPart.CFrame = rootPart.CFrame * CFrame.new(0, 20, 0)
                                        Tween(CFrame.new(-10160.787, 138.662, 5955.031))
                                        game:GetService("VirtualUser"):CaptureController()
                                        game:GetService("VirtualUser"):Button1Down(Vector2.new(1280, 672))
                                    end
                                end
                        ...
                        Tween(CFrame.new(-9681.458, 6.14, 6341.372))
                    end
                end
            end)
        end
    ...
    while true do
        if _G.AutoY then
            pcall(function()
                ...
                wait(0.1); -- Short press
                game:GetService("VirtualInputManager"):SendKeyEvent(false, "Y", false, game);
                wait(0.5); -- Wait between presses to avoid input flooding
            end);
        else
            wait(0.1); -- Reduced CPU usage when not active
        end
    end
end);
...
Description = "White Your All Screen",
...
v16.Player:AddButton({
    ...
    Description = "",
    Callback = function()
        table.clear(v112) -- Clears the player list
        for _, player in ipairs(game:GetService("Players"):GetPlayers()) do
            table.insert(v112, player.Name)
        end
        print("Players refreshed:", table.concat(v112, ", ")) -- Debug output
...
Description = "Target Selected Player And Manual Use Your Skils",
...
RaidToggle:OnChanged(function(v238)  -- Changed from v50 to RaidToggle
    _G.AutoNear = v238;
...
RaidToggle:SetValue(false);  -- Changed from v17.ToggleMobAura to RaidToggle
...
Description = "Instant Kill Aura Mobs",
...
local RaceToggle = v16.Race:AddToggle("ToggleMobAura", {
    Title = "Auto Trial Ghoul / Human",
    Description = "Like Auto Farming Version",
    Default = false
});

RaceToggle:OnChanged(function(v238)  -- Changed from v50 to RaceToggle
    _G.AutoNear = v238;
...
RaceToggle:SetValue(false);  -- Changed from v17.ToggleMobAura to RaceToggle
                ...
                for v838, v839 in pairs(game.Workspace.Enemies:GetChildren()) do
                    if (v839:FindFirstChild("Humanoid") and v839:FindFirstChild("HumanoidRootPart") and (v839.Humanoid.Health > 0)) then
                        if v839.Name then
                            if ((game.Players.LocalPlayer.Character.HumanoidRootPart.Position - v839:FindFirstChild("HumanoidRootPart").Position).Magnitude <= 5000) then
                                repeat
                                    wait(_G.Fast_Delay or 0);  -- Added default value in case _G.Fast_Delay is nil
                                    
                                    -- Make sure these functions exist
                                    if AttackNoCoolDown then AttackNoCoolDown(); end
                                    
                                    bringmob = true;
                                    
                                    if AutoHaki then AutoHaki(); end
                                    if EquipTool and SelectWeapon then EquipTool(SelectWeapon); end
                                    
                                    -- Make sure Pos and Tween are defined
                                    local Pos = Pos or CFrame.new(0, 30, 0)  -- Default position if Pos is nil
                                    if Tween then
                                        Tween(v839.HumanoidRootPart.CFrame * Pos);
                                    end
...
v158:OnChanged(function(v388)
    ...
    if _G.AutoReceiveQuest then
        -- Move to entrance position for quest
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(5661.5322265625, 1013.0907592773438, -334.9649963378906));
        Tween2(CFrame.new(5814.42724609375, 1208.3267822265625, 884.5785522460938));  -- Assuming Tween2 moves the player to the quest location.

        -- Handling quest acceptance and check
        spawn(function()
            pcall(function()
                while wait(1) do  -- Wait time adjusted to 1 second for smoother performance.
                    if _G.AutoReceiveQuest then
                        -- Request the quest
                        local requestQuest = {
                            [1] = {
                        ...
                        -- Check if the quest is successfully accepted
                        local checkQuest = {
                            [1] = {
                                Context = "Check"
                            }
                        };
                        local result = game:GetService("ReplicatedStorage").Modules.Net:FindFirstChild("RF/DragonHunter"):InvokeServer(unpack(checkQuest));

                        -- If quest is accepted, perform necessary actions to complete it
                        if result then
                            -- Perform quest completion logic (replace with specific quest actions)
                            -- For example, interact with certain objects, defeat enemies, etc.

                            -- Example: Assuming the quest requires interacting with an NPC
                            -- If your quest requires actions like defeating enemies, use additional logic to perform those actions.

                            -- Here is an example of completing an NPC interaction:
                            local interactNpc = {
                                [1] = {
                                    Context = "InteractNPC"
                                }
                            };
                            game:GetService("ReplicatedStorage").Modules.Net:FindFirstChild("RF/DragonHunter"):InvokeServer(unpack(interactNpc));
                            
                            -- Or if it’s about defeating enemies or specific actions, do it here
                            -- Assuming you have a method to defeat the enemies (this is an example, modify it accordingly)
                            -- local defeatEnemies = {
                            --     [1] = {
                            --         Context = "DefeatEnemies"
                            --     }
                            -- };
                            -- game:GetService("ReplicatedStorage").Modules.Net:FindFirstChild("RF/DragonHunter"):InvokeServer(unpack(defeatEnemies));
                        end
                    else
                        break  -- Stop the loop if the toggle is turned off
                    end
                end
            end);
        end);
...
local v169 = v16.Volcano:AddParagraph({
...
local v170 = v16.Volcano:AddToggle("ToggleTPVolcano", {
...
local v171 = v16.Volcano:AddToggle("ToggleDefendVolcano", {
...
local v107 = v16.Volcano:AddToggle("ToggleMelee", {
...
local v109 = v16.Volcano:AddToggle("ToggleSword", {
...
local v110 = v16.Volcano:AddToggle("ToggleGun", {
...
local v175 = v16.Volcano:AddToggle("ToggleKillAura", {

Title = "Auto Instant Kill Golem",

Description = "Golem Instant Dead When It's Spawn",
...
local v176 = v16.Volcano:AddToggle("ToggleCollectBone", {
...
local v177 = v16.Volcano:AddToggle("ToggleCollectEgg", {
...
local FastAttackToggle = v16.Hunt:AddToggle("FastAttackToggle", {
    Title = "Auto Fast Attack",
    Default = true -- Enabled by default
})

_G.FastAttack = true -- Auto-enable Fast Attack
...
-- Start Fast Attack loop automatically
spawn(function()
    while _G.FastAttack do
        task.wait(0.1)
        pcall(_x1)
...
-- Add button instead of toggle
v16.Player:AddButton({
    Title = "Hop to Lowest Server",
    Description = "",
    Callback = function()
        HopToLowestServer()
    end
...
function HopToLowestServer()
    ...
    repeat
        ...
        task.wait(1) -- Added delay to avoid rate limiting
    until cursor == "" or foundServer
...
-- Create the Aimbot Camera Button
v16.Player:AddButton({
    Title = "Aimbot Camera",
    Description = "",
    Callback = function()
...
-- Seabeast
local AutoKillSeabeast = v16.Event:AddToggle("ToggleAutoKillSeabeast", { 
    Title = "Auto Kill Sea Beast", 
    Description = "", 
    Default = false 
})

AutoKillSeabeast:OnChanged(function(state) 
    if state then 
        spawn(function() 
            local player = game.Players.LocalPlayer
            ...
            while AutoKillSeabeast.Value do
                for _, tool in pairs(backpack:GetChildren()) do
                    if tool:IsA("Tool") and (tool.ToolTip == "Melee" or tool.ToolTip == "Blox Fruit" or tool.ToolTip == "Sword" or tool.ToolTip == "Gun") then
                        ...
                        virtualInput:SendKeyEvent(true, 118, false, player.Character.HumanoidRootPart) -- V
                        virtualInput:SendKeyEvent(false, 118, false, player.Character.HumanoidRootPart)
                        wait(0.2)
                    end
                end
                wait()
...
local SkillSection = v16.Levi:AddSection("Leviathan Mobs (Beta)");

local AutoKillLeviathan = v16.Levi:AddToggle("ToggleAutoKillLeviathan", {
    Title = "Auto Kill Leviathan",
    Description = "",
    Default = false
})

AutoKillLeviathan:OnChanged(function(state)
    if state then
        -- Movement
        spawn(function()
            local player = game.Players.LocalPlayer
            ...
            local leviathan = game:GetService("Workspace").SeaBeasts:FindFirstChild("Leviathan")
            if leviathan and humanoidRootPart then
                local rootPart = leviathan:FindFirstChild("HumanoidRootPart")
                if rootPart then
                    local initialTargetPosition = rootPart.Position + Vector3.new(0, 400, -10)
                    Tween(CFrame.new(initialTargetPosition))

                    while AutoKillLeviathan.Value do
                        local currentY = humanoidRootPart.Position.Y
                        local currentXZ = Vector3.new(humanoidRootPart.Position.X, 0, humanoidRootPart.Position.Z)
...
local AutoKillSegmebtLeviathan = v16.Levi:AddToggle("ToggleAutoKillLeviathan", {
    Title = "Auto Kill Segment Leviathan",
    Description = "",
    Default = false
})
    ...
    if state then
        spawn(function()
            ...
            for _, segmentName in ipairs(segments) do
                if not AutoKillLeviathan.Value then break end

                local segment = game:GetService("Workspace"):FindFirstChild(segmentName, true)
                if segment and segment:FindFirstChild("HumanoidRootPart") then
                    local rootPart = segment.HumanoidRootPart
                    local initialTargetPosition = rootPart.Position + Vector3.new(0, 400, -10)

                    Tween(CFrame.new(initialTargetPosition))

                    while AutoKillLeviathan.Value and segment.Parent do
                        local currentY = humanoidRootPart.Position.Y
                        local targetXZ = Vector3.new(rootPart.Position.X, 0, rootPart.Position.Z)
                        local newTargetPosition = Vector3.new(targetXZ.X, initialTargetPosition.Y, targetXZ.Z - 10)
            ...
            while AutoKillLeviathan.Value do
                for _, tool in pairs(backpack:GetChildren()) do
                    if tool:IsA("Tool") and (tool.ToolTip == "Melee" or tool.ToolTip == "Blox Fruit" or tool.ToolTip == "Sword" or tool.ToolTip == "Gun") then
                        ...
                        local virtualInput = game:GetService("VirtualInputManager")
                        for _, key in ipairs({122, 120, 99, 118}) do -- Z, X, C, V
                            virtualInput:SendKeyEvent(true, key, false, player.Character.HumanoidRootPart)
                            virtualInput:SendKeyEvent(false, key, false, player.Character.HumanoidRootPart)
                            wait(0.2)
                        end
                    end
                end
...
v16.Draco:AddButton({
    Title = "Viper Fly GUI",
    Description = "Use For Draco Trial Ez Win",
    Callback = function()
...
Title = "Nova Hub Goddess",

Content = "Nova Hub Loaded",
