...
Main:AddDropdown({
    Name = "Logo Scale",
    Flag = "Main/LogoSize",  -- Set a unique flag for easy reference
    Default = "Medium",  -- Set the default size
    Options = { "Small", "Medium", "Large" },  -- The options for logo size
    Callback = function(option)
        -- Set the size based on the selected option
        ...
        elseif option == "Large" then
            v23.Size = UDim2.new(0, 100, 0, 100)  -- Large size
        end
    end
})
    ...
    while task.wait() do
        if _G.AutoFarmBones then
            pcall(function()
                ...
                -- Scan for any Bone mob in the entire workspace
                local foundMob = false
                local targetMob = nil
                ...
                if foundMob and targetMob then
                    ...
                    Tween(targetMob.HumanoidRootPart.CFrame * CFrame.new(0, 50, 0))
                    AttackNoCoolDown()
                end
...
Setting:AddSlider({
    ...
    Flag = "Setting/BringMobDistance",
    Description = "Distance to bring mobs towards you",
    Range = {1, 100},
    Suffix = "",
    CurrentValue = 50,
    Increment = 1,
    Callback = function(Value)
        getgenv().LocalConfig = getgenv().LocalConfig or {}
        getgenv().LocalConfig.Settings = getgenv().LocalConfig.Settings or {}
        getgenv().LocalConfig.Settings.Script = getgenv().LocalConfig.Settings.Script or {}
        getgenv().LocalConfig.Settings.Script["Bring Mob Distance"] = Value
    end,
})
...
local bringMob = newcclosure(function(mobName)
    ...
    for _, v in ipairs(workspace.Enemies:GetChildren()) do  
        ...
        if v:IsA("Model") and humanoid and npcRoot and not v.Name:find("King") and not v.Name:find("Boss") then  
            if (npcRoot.Position - rootPart.Position).Magnitude <= getgenv().LocalConfig.Settings.Script["Bring Mob Distance"] then  
                npcRoot.CFrame = rootPart.CFrame * CFrame.new(0, -20, -10)  
        ...
        if _G.BoneFarmV4 then
            pcall(function()
                ...
                if not hasTeleportedToGraveIsland then
                    -- Change the teleport position to Grave Island
                    Tween(CFrame.new(-9480.80762, 142.130661, 5566.37305, -0.00655503059, 4.5295423e-8, -0.999978542, 2.0492047e-8, 1, 4.5162068e-8, 0.999978542, -2.0195568e-8, -0.00655503059))
...
-- Toggle for Auto Find Mirage Island
Sea:AddToggle({
    Name = "Auto Find Mirage Island",
    Flag = "Sea/AutoFindMirage",
...
-- Initialize AutoFindMirage to false
Sea.AutoFindMirage:SetValue(false);

-- RenderStepped function to constantly check for Mirage Island
game:GetService("RunService").RenderStepped:Connect(function()
    ...
    local function findAvailableSeat()
        ...
        for _, boat in pairs(v511) do
            if boat and boat.Parent and boat.Name == "VehicleSeat" and not boat.Occupant then
                Tween2(boat.CFrame);
                break;
            end
        end
    ...
    for _, boat in pairs(game.Workspace.Boats:GetChildren()) do
        ...
        if vehicleSeat and vehicleSeat.Occupant == humanoid then
            ...
            v511[boat.Name] = vehicleSeat;
        elseif vehicleSeat and not vehicleSeat.Occupant then
            findAvailableSeat();
        end
    end
    ...
    boatSeat.MaxSpeed = v508;
    boatSeat.CFrame = CFrame.new(Vector3.new(boatSeat.Position.X, boatSeat.Position.Y, boatSeat.Position.Z)) * boatSeat.CFrame.Rotation;

    game:GetService("UserInputService"):SendKeyEvent(true, "W", false, game);

    -- Disable collisions for all parts of the boat and character
    ...
    local islands = {
        "ShipwreckIsland", "SandIsland", "TreeIsland", "TinyIsland", "PrehistoricIsland", "KitsuneIsland", "FrozenDimension"
    };

    for _, island in ipairs(islands) do
        local islandModel = game.Workspace.Map:FindFirstChild(island);
    ...
    if mysticIsland then
        game:GetService("UserInputService"):SendKeyEvent(false, "W", false, game);
        _G.AutoFindMirage = false;

        if not v513 then
            game:GetService("ReplicatedStorage"):FireServer({
                Title = "Viper Hub Notify",
...
-- Slider for Boat Speed
Sea:AddSlider({
    Name = "Boat Speed",
    Flag = "Sea/BoatSpeed",
    Description = "Adjust the speed of your boat",
    Default = v508, -- v508 should be initialized or defined earlier
    Min = 0,
    Max = 350,
    Rounding = 1,
    Callback = function(Value)
        v508 = Value; -- Update the boat speed
    end
});
...
function Tween(targetCFrame)
    ...
    if distance > 200 and distance < 1000 or distance > 2500 then
        root.CFrame = root.CFrame + (targetCFrame.Position - root.Position).Unit * 600
    elseif distance < 500 then
        root.CFrame = targetCFrame
    else
        root.CFrame = root.CFrame + (targetCFrame.Position - root.Position).Unit * 200
    end
