...
-- Initialize weapon selection variables
_G.WeaponCategory = "Melee"
_G.SelectWeapon = ""
...
Main:AddDropdown({
    ...
    Default = "Melee",
    Flag = "AutoFarm/SelectWeapon",
    Callback = function(Value)
        _G.WeaponCategory = Value
        print("Weapon Category Changed to: " .. Value)
    end
})

-- Custom EquipWeapon function for proper weapon selection and equipping
function EquipWeapon(weaponCategory)
    local player = game.Players.LocalPlayer
    local backpack = player.Backpack
    local character = player.Character
    
    -- First try to use any already equipped weapon of the right category
    local currentEquipped = character:FindFirstChildOfClass("Tool")
    if currentEquipped and currentEquipped:FindFirstChild("ToolTip") then
        if (weaponCategory == "Melee" and currentEquipped.ToolTip == "Melee") or
           (weaponCategory == "Sword" and currentEquipped.ToolTip == "Sword") or
           (weaponCategory == "Gun" and currentEquipped.ToolTip == "Gun") or
           (weaponCategory == "Fruit" and currentEquipped.ToolTip == "Blox Fruit") then
            -- Already equipped weapon of right category
            return true
        end
    end
    
    -- If no matching equipped weapon, check backpack
    for _, v in pairs(backpack:GetChildren()) do
        if v:FindFirstChild("ToolTip") then
            if (weaponCategory == "Melee" and v.ToolTip == "Melee") or
               (weaponCategory == "Sword" and v.ToolTip == "Sword") or
               (weaponCategory == "Gun" and v.ToolTip == "Gun") or
               (weaponCategory == "Fruit" and v.ToolTip == "Blox Fruit") then
                -- Found matching weapon in backpack, equip it
                character.Humanoid:EquipTool(v)
                print("Equipped " .. v.Name .. " (" .. v.ToolTip .. ")")
                return true
            end
        end
    end
    
    print("No weapon found matching category: " .. weaponCategory)
    return false
end
                    ...
                    for _, enemy in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        if enemy:FindFirstChild("Humanoid") and enemy:FindFirstChild("HumanoidRootPart") and enemy.Humanoid.Health > 0 then
                            if string.find(enemy.Name, Ms) then
                                ...
                                AutoHaki()
                                -- Use the weapon category directly
                                EquipWeapon(_G.WeaponCategory)
    ...
    while task.wait() do
        if _G.AutoFarmTyrant then
            pcall(function()
                ...
                -- Only teleport once when starting the farm
                if initialTeleport then
                ...
                -- Farm the Tyrant without repeated teleporting
                ...
                -- If no mob is found, wait a bit before checking again
                if not mobFound then
                    task.wait(1)
                end
            end)
        else
            -- Reset the initialTeleport flag when turning off auto farm
...
Sea:AddToggle({
    ...
    Description = "<font color='rgb(255, 0, 0)'>This Drive Doesn't Support Adjust Speed This is Default 350 Speed</font>",
    Default = false,
    Callback = function(Value)
...
Sea:AddParagraph({"Auto Kill Sea Mobs", "Coming Soon"})
