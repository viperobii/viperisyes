...
Title = "Nova Cat Hub BloxFruit [ Freemium ]",
...
elseif ((v197 >= 2575 and v197 <= 2650) or (SelectMonster == "Skull Slayer")) then
    Ms = "Skull Slayer"
    NameQuest = "TikiQuest3"
    QuestLv = 2
...
local ChestSec = v16.Main:AddSection("Chests")
...
local InstantChest = v16.Main:AddToggle("ToggleInstantChest", {

    Title = "Auto Farm Chest (Instant)",
    Description = "Instant Collect Chest, Will Reset If No Chest Found In A Sec",
    Default = false,
...
Tween(CFrame.new(v1033.HumanoidRootPart.Position + Vector3.new(0, 30, 0)));
...
Tween(CFrame.new(v843.HumanoidRootPart.Position + Vector3.new(0, 30, 0)));
...
spawn(function()
    while task.wait(0.1) do
        if AutoFarmMasDevilFruit and TypeMastery == "Farm Level" then
            pcall(function()
                CheckLevel()
                local player = game:GetService("Players").LocalPlayer
                local questGui = player.PlayerGui.Main.Quest
                ...
                if not string.find(questGui.Container.QuestTitle.Title.Text, NameMon) or not questGui.Visible then
                    ...
                    for _, enemy in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        if enemy.Name == Ms and enemy:FindFirstChild("Humanoid") and enemy:FindFirstChild("HumanoidRootPart") then
                            if (enemy.Humanoid.Health > 0) then
                                repeat task.wait(_G.Fast_Delay)
                                    ...
                                    if (enemy.Humanoid.Health <= ((enemy.Humanoid.MaxHealth * KillPercent) / 100)) then
                                        ...
                                        -- Mob lock logic
                                        Tween(enemy.HumanoidRootPart.CFrame * CFrame.new(0, 30, 0))
                                        enemy.HumanoidRootPart.Size = Vector3.new(60, 60, 60)
                                        enemy.HumanoidRootPart.Transparency = 1
                                        enemy.Humanoid.JumpPower = 0
                                        enemy.Humanoid.WalkSpeed = 0
                                ...
                                until not AutoFarmMasDevilFruit or TypeMastery ~= "Farm Level" or not enemy.Parent or enemy.Humanoid.Health <= 0
