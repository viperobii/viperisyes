...
Main:AddDropdown({
    ...
    Callback = function(option)
        local button = getgenv().LogoButton
...
Main:AddToggle({
    ...
    Callback = function(Value)
        ...
        if not Value then
            wait()
            -- Ensure player is above the water level
            local player = game:GetService("Players").LocalPlayer
            local playerPosition = player.Character.HumanoidRootPart.Position
            if playerPosition.Y <= 0 then
                player.Character.HumanoidRootPart.CFrame = CFrame.new(playerPosition.X, 5, playerPosition.Z)
            end
        ...
        if _G.AutoLevel then
            pcall(function()
                ...
                if not string.find(questGui.Container.QuestTitle.Title.Text, NameMon) or not questGui.Visible then
                    ...
                    -- Ensure player stays above water after tweening
                    local playerPosition = player.Character.HumanoidRootPart.Position
                    if playerPosition.Y <= 0 then
                        player.Character.HumanoidRootPart.CFrame = CFrame.new(playerPosition.X, 5, playerPosition.Z)
                    end
                    ...
                    -- Check if there are mobs to farm
                    local foundMob = false
                    
                    for _, enemy in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        if enemy:FindFirstChild("Humanoid") and enemy:FindFirstChild("HumanoidRootPart") and enemy.Humanoid.Health > 0 then
                            if string.find(enemy.Name, Ms) then
                                foundMob = true
                                
                                -- Target and attack the mob
                                AutoHaki()
                    ...
                    -- If no mobs found, go to spawn location
                    if not foundMob then
                        bringmob = false
                        -- Go to mob spawn location
                        Tween(CFrameMon)

                        -- Ensure player stays above water during movement
                        local playerPosition = player.Character.HumanoidRootPart.Position
                        if playerPosition.Y <= 0 then
                            player.Character.HumanoidRootPart.CFrame = CFrame.new(playerPosition.X, 5, playerPosition.Z)
                        end
...
Main:AddToggle({
    ...
    Flag = "Main/AutoFarmNearestMob",
    Description = "Nearest Mob",
    Default = false,
    Callback = function(Value)
        _G.AutoNear = Value
        if not Value then bringmob = false end
    end
})

-- AUTO FARM NEAREST MOB LOOP
spawn(function()
    while task.wait() do
        if _G.AutoNear then
            pcall(function()
                ...
                if Closest then
                    repeat
                        ...
                        AutoHaki()
                        EquipTool(SelectWeapon)
                        Tween(CFrame.new(Closest.HumanoidRootPart.Position + Vector3.new(0, 20, 0)))
                        AttackNoCoolDown()
                        if Closest:FindFirstChild("HumanoidRootPart") then
                            Closest.HumanoidRootPart.Size = Vector3.new(60, 60, 60)
                            Closest.HumanoidRootPart.Transparency = 1
                        ...
                        bringmob = true
                    until not _G.AutoNear or Closest.Humanoid.Health <= 0 or not Closest.Parent
                    bringmob = false
                end
            end)
        end
    ...
    while wait() do
        if _G.AutoCollectChest then
            pcall(function()
                ...
                local chestTag = game:GetService("CollectionService"):GetTagged("_ChestTagged")
                local closestDistance = math.huge
                local closestChest = nil

                for _, chest in pairs(chestTag) do
                    local dist = (chest:GetPivot().Position - rootPos).Magnitude
                    if not chest:GetAttribute("IsDisabled") and dist < closestDistance then
                        closestDistance = dist
                        closestChest = chest
                    end
                end
                ...
                if closestChest then
                    Tween2(CFrame.new(closestChest:GetPivot().Position))
                end
            end)
        end
    end
...
Main:AddToggle({
    Name = "Auto Farm Chest (Instant TP)",
    Flag = "Main/ChestBypass",
    Description = "Automatically teleports to the nearest chest instantly.",
    Default = false,
    Callback = function(Value)
    ...
    while task.wait() do
        if _G.ChestBypass then
            ...
            while (tick() - startTime) < 4 do
                ...
                for i = 1, #taggedChests do
                    local chest = taggedChests[i]
                ...
                if closestChest then
                    local chestPos = closestChest:GetPivot()
                    BTPZ(chestPos) -- INSTANT TP
                    task.wait(0.2)
                else
                    break
                end
...
local hasTeleportedToBoneIsland = false

Main:AddToggle({
    Name = "Auto Farm Bones",
    Flag = "Main/AutoFarmBones",
    Description = "",
    Default = false,
    Callback = function(Value)
        _G.AutoFarmBones = Value
        if Value == false then
            hasTeleportedToBoneIsland = false -- Reset when turning off
        end
    end
})
...
spawn(function()
    ...
    local mobsCleared = {false, false, false, false}  -- Track if each mob type is cleared
    
    -- List of mobs to farm in order
    local mobsToFarm = {
        "Reborn Skeleton",
        "Living Zombie",
        "Posessed Mummy",
    ...
    while task.wait() do
        if _G.AutoFarmBones then
            pcall(function()
                -- First teleport to Bone Island once when enabled
                if not hasTeleportedToBoneIsland then
                    Tween(CFrame.new(-9480.80762, 142.130661, 5566.37305, -0.00655503059, 4.5295423e-8, -0.999978542, 2.0492047e-8, 1, 4.5162068e-8, 0.999978542, -2.0195568e-8, -0.00655503059))
                    hasTeleportedToBoneIsland = true
                    task.wait(1) -- Wait for teleport to complete
                end
                
                -- Get current mob to farm
                local currentMobName = mobsToFarm[currentMobIndex]
                
                -- Check if current mob exists
                local targetMob = nil
                for _, enemy in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                    if enemy.Name == currentMobName and enemy:FindFirstChild("Humanoid") and 
                ...
                -- If current mob exists, attack it
                if targetMob then
                    -- Mark this mob type as not cleared since we found one
                    mobsCleared[currentMobIndex] = false
                    ...
                    Tween(targetMob.HumanoidRootPart.CFrame * CFrame.new(0, 50, 0)) -- Adjust position
                    AttackNoCoolDown()
                else
                    -- Mark this mob type as cleared since we couldn't find any
                    mobsCleared[currentMobIndex] = true
                    
                    -- Move to next mob type
                    currentMobIndex = currentMobIndex + 1
                    
                    -- If we've checked all mob types, restart from beginning only if all are cleared
                    if currentMobIndex > #mobsToFarm then
                        -- Check if all mobs were cleared
                        local allCleared = true
                        for i = 1, #mobsCleared do
                            if not mobsCleared[i] then
                                allCleared = false
                                break
                            end
                        end
                        
                        -- If all were cleared, reset and start from the first mob again
                        if allCleared then
                            currentMobIndex = 1
                            -- Reset cleared status for all mobs
                            for i = 1, #mobsCleared do
                                mobsCleared[i] = false
                            end
                            -- Wait a bit for mobs to respawn
                            task.wait(3)
                        else
                            -- If not all were cleared, restart from the first mob that wasn't cleared
                            for i = 1, #mobsCleared do
                                if not mobsCleared[i] then
                                    currentMobIndex = i
                                    break
                    ...
                    -- Add a small delay before trying the next mob
                    task.wait(0.5)
                end
            end)
        else
            -- Reset variables when script is disabled
            hasTeleportedToBoneIsland = false
            currentMobIndex = 1
            for i = 1, #mobsCleared do
                mobsCleared[i] = false
            end
...
Main:AddToggle({
    ...
    Flag = "Main/AutoRollBones",
    Description = "Auto Spin Bone",
    Default = false,
    Callback = function(Value)
        _G.AutoRollBones = Value
        if Value then
            spawn(function()
                while _G.AutoRollBones do
                    pcall(function()
                        local args = {
                            [1] = "Bones",
                            [2] = "Buy",
        ...
        if _G.AutoFarmTyrant then
            pcall(function()
                ...
                repeat
                    ...
                    task.wait()
                until not mobFound or not _G.AutoFarmTyrant
            end)
        end
    end
end)
...
local hasTeleportedToTikiIsland = false

Main:AddToggle({
    Name = "Auto Farm All Tiki Mobs",
    Flag = "Main/AutoFarmAllTikiMobs",
    Description = "",
    Default = false,
    Callback = function(Value)
        _G.AutoFarmTiki = Value
        if Value == false then
            hasTeleportedToTikiIsland = false -- Reset when turning off
        end
    end
})
...
spawn(function()
    while task.wait() do
        if _G.AutoFarmTiki then
            pcall(function()
                -- First teleport to Tiki Island once when enabled
                if not hasTeleportedToTikiIsland then
                    Tween(CFrame.new(-16314, 21, 1085))
                    hasTeleportedToTikiIsland = true
                    task.wait(1) -- Wait for teleport to complete
                end
                ...
                -- Scan for any Tiki mob in the entire workspace
                local foundMob = false
                local targetMob = nil
                ...
                -- If we found a mob, attack it
                if foundMob and targetMob then
                    AutoHaki()
                    EquipTool(_G.SelectWeapon or SelectWeapon)
                    targetMob.HumanoidRootPart.CanCollide = false
                    targetMob.Humanoid.WalkSpeed = 0
                ...
                -- Brief wait if no mob was found to prevent excessive looping
                if not foundMob then
                    task.wait(1)
                end
            end)
...
Main:AddSection("Cake")

Main:AddToggle({
    Name = "Auto Farm Dough King",
    Flag = "Main/AutoFarmDoughKing",
    Description = "",
    Default = false,
    Callback = function(Value)
        _G.AutoFarmDoughKing = Value
    end
})
    ...
    while task.wait() do
        if _G.AutoFarmDoughKing then
            pcall(function()
                ...
                -- Teleport once to area near Dough King
                if not hasTweened then
                    Tween(CFrame.new(-2021, 123, -12194)) -- adjust if needed
                    hasTweened = true
                end

                for _, mobName in ipairs(mobs) do
                    if not _G.AutoFarmDoughKing then break end
                    ...
                    repeat
                        ...
                        task.wait()
                    until not mobFound or not _G.AutoFarmDoughKing
                end
            end)
        else
            hasTweened = false
    ...
    while task.wait() do
        if _G.AutoFarmCakePrince then
            pcall(function()
                ...
                if not hasTweened then
                    Tween(CFrame.new(-2015, 122, -12318)) -- Adjust if Cake Prince location is different
                    hasTweened = true
                end

                for _, mobName in ipairs(mobs) do
                    if not _G.AutoFarmCakePrince then break end
                    ...
                    repeat
                        ...
                        task.wait()
                    until not mobFound or not _G.AutoFarmCakePrince
                end
            end)
        else
            hasTweened = false
...
Main:AddToggle({
    Name = "Auto Farm Cake Island Mobs",
    Flag = "Main/AutoFarmCakeIslandMobs",
    Description = "Farms all mobs in Cake Island",
    Default = false,
    Callback = function(Value)
        _G.AutoFarmCakeIsland = Value
        hasTweened = false -- reset so it teleports once when toggled back on
    end
})

spawn(function()
    while task.wait() do
        if _G.AutoFarmCakeIsland then
            pcall(function()
                local mobs = {
                    "Baking Staff",
                    "Head Baker",
                ...
                if not hasTweened then
                    Tween(CFrame.new(-2021, 123, -12194)) -- tp once
                    hasTweened = true
                end

                for _, mobName in ipairs(mobs) do
                    if not _G.AutoFarmCakeIsland then break end
                    ...
                    repeat
                        ...
                        task.wait()
                    until not mobFound or not _G.AutoFarmCakeIsland
                end
            end)
        end
    end
...
Main:AddToggle({
    ...
    Flag = "Main/AutoSummonCakePrince",
...
spawn(function()
    while wait() do
        if _G.SpawnCakePrince then
            pcall(function()
                -- Assuming "CakePrinceSpawner" is the object that spawns the Cake Prince
                local spawnData = {
                    [1] = "CakePrinceSpawner",
                    [2] = true -- This will trigger the spawn
                }
                
                -- You may need to change this to the actual code that triggers the spawn
                local spawner = game.Workspace:FindFirstChild("CakePrinceSpawner")
                if spawner then
                    -- Call the function or event to spawn the Cake Prince
                    spawner:Fire(spawnData) -- Replace with actual method to spawn
                end
            end)
        end
    end
...
Player:AddDropdown({
    Name = "Select Player",
    Flag = "Player/SelectPlayers",
    Description = "Choose a player to target",
    Options = PlayersList,
    Callback = function(selectedPlayer)
        _G.SelectPly = selectedPlayer
    end
})
...
Player:AddButton({
    ...
    Callback = function()
        -- Refresh the PlayersList
        PlayersList = {}
        for _, player in pairs(game:GetService("Players"):GetChildren()) do
            table.insert(PlayersList, player.Name)
        end

        -- Clear current dropdown options and add new ones
        playerDropdown:SetOptions(PlayersList)
    end
})

-- Toggle for Auto Kill Player
Player:AddToggle({
    Name = "Auto Kill Player",
    Flag = "Player/AutoKillPlayer",
...
-- Real-time tween follow loop
spawn(function()
    while task.wait(0.1) do
        pcall(function()
            if _G.TeleportPly and _G.SelectPly and game.Players:FindFirstChild(_G.SelectPly) then
                local target = game.Players[_G.SelectPly]
                if target.Character and target.Character:FindFirstChild("HumanoidRootPart") then
                    AutoHaki()
                    Tween(target.Character.HumanoidRootPart.CFrame * CFrame.new(0, 0, -3)) -- optional offset
                end
            end
        end)
    end
...
Player:AddToggle({
    ...
    Callback = function(Value)
        ...
        if _G.NoClip and not _G.NoClipLoop then
        ...
        if not _G.NoClip and _G.NoClipLoop then
            local char = game.Players.LocalPlayer.Character
            if char and char:FindFirstChild("HumanoidRootPart") then
                for _, part in pairs(char:GetDescendants()) do
                    if part:IsA("BasePart") then
...
Player:AddSection("Aim")

Player:AddToggle({
    Name = "Aimbot Slill Nearest",
    Flag = "Player/Nearest",
    Default = false,
    Callback = function(Value)
        _G.AimNearest = Value
...
-- Add this function to format numbers (replacement for v20)
...
Setting:AddSection("Player Setting")


Setting:AddDropdown({
    Name = "Bring Mob Mode",
    Flag = "BringMobModeDropdown",
    Description = "",
    Options = {"Low", "Normal", "Super Bring"},
    Default = "Low",
    Callback = function(value)
        -- Directly set the numeric value based on dropdown selection
        if value == "Low" then
            _G.BringModeDistance = 250
        elseif value == "Normal" then
            _G.BringModeDistance = 300
        elseif value == "Super Bring" then
            _G.BringModeDistance = 350
        end
    end
})

-- Add Toggle for Bring Mobs
Setting:AddToggle({
    Name = "Bring Mobs",
    Flag = "BringMobsToggle",
    Description = "<font color='rgb(255, 0, 0)'>Sometimes Work & Sometimes Not</font>",
    Default = false,
    Callback = function(Value)
...
-- Spawn function to check and bring monsters to the player
spawn(function()
    while task.wait() do
        pcall(function()
            if _G.BringMonster then
                ...
                for v2110, v2111 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                    if (_G.Level and StartMagnet and (v2111.Name == Mon) and
                        ((Mon == "Factory Staff [Lv. 800]") or
 (Mon == "Monkey [Lv. 14]") or
 (Mon == "Dragon Crew Warrior [Lv. 1575]") or
 (Mon == "Dragon Crew Archer [Lv. 1600]") or
 (Mon == "Reborn Skeleton [Lv. 1975]")) and
                        v2111:FindFirstChild("Humanoid") and
                        v2111:FindFirstChild("HumanoidRootPart") and
                        (v2111.Humanoid.Health > 0) and
                        ((v2111.HumanoidRootPart.Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 220)) then
                    ...
                    elseif (_G.Level and StartMagnet and v2111.Name == Mon and
                        v2111:FindFirstChild("Humanoid") and
                        v2111:FindFirstChild("HumanoidRootPart") and
                        (v2111.Humanoid.Health > 0) and
                        ((v2111.HumanoidRootPart.Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= _G.BringModeDistance)) then
                        
                        -- Regular teleportation (based on the selected BringMode)
                        v2111.HumanoidRootPart.Size = Vector3.new(50, 50, 50)
                        v2111.HumanoidRootPart.CFrame = PosMon
                        v2111.Humanoid:ChangeState(14)
                        v2111.HumanoidRootPart.CanCollide = false
...
Items:AddSection("Stats");

Items:AddToggle({
    Name = "Auto Stats Melee",
    Flag = "Items/AutoStatsMelee",
    Description = "",
    Default = false,
    Callback = function(Value)
...
Items:AddToggle({
    Name = "Auto Stats Swords",
    Flag = "Items/AutoStatsSword",
    Description = "",
    Default = false,
    Callback = function(Value)
...
Setting:AddSection("Miscellaneous")

Setting:AddToggle({
    Name = "Remove Fog (Sea 3)",
    Flag = "Setting/RemoveFog",
    Default = false,
    Callback = function(Value)
        if Value then
...
Setting:AddToggle({
    Name = "Enable Remove Notifications",
    Flag = "Setting/RemoveNotifications",
    Description = "",
    Callback = function(value)
        spawn(function()
            while value do
...
Trial:AddToggle({
    Name = "Teleport NPC Great Tree",
    Flag = "Trial/TeleportNPCGreatTree",
    Description = "<font color='rgb(0, 255, 0)'>TP To NPC Top Tree</font>",
    Default = false,
    Callback = function(Value)
        if Value then
            Tween2(CFrame.new(3032.78003, 2282.35107, -7325.47803, -0.969380021, 0, 0.2455661, 0, 1, 0, -0.2455661, 0, -0.969380021))        
...
Trial:AddToggle({
    Name = "Teleport To TempleClock",
    Flag = "Trial/TeleportTempleClock",
    Description = "",
    Default = false,
    Callback = function(Value)
        if Value then
...
Trial:AddToggle({
    ...
    Description = "Enable or Disable teleport to Race Door",
    Default = false,
    Callback = function(Value)
        if Value then
            for _, v in ipairs(game.ReplicatedStorage.MapStash:GetChildren()) do
...
Trial:AddSection("Trial")

-- Add toggle for Auto Trial
Trial:AddToggle({
    Name = "Auto Trial",
    Flag = "Trial/AutoTrial",
    Description = "For Ghoul Trial, Double Jump First And Turn On Auto Trial (Instant Kill Mobs)",
    Default = false,
    Callback = function(Value)
        ...
        if _G.AutoQuestRace then
            pcall(function()
                    ...
                    for _, mob in pairs(game.Workspace.Enemies:GetDescendants()) do
                        if mob:FindFirstChild("Humanoid") and mob:FindFirstChild("HumanoidRootPart") and mob.Humanoid.Health > 0 then
                            pcall(function()
                                ...
                                until not _G.AutoQuestRace or not mob.Parent or mob.Humanoid.Health <= 0
                            end)
                        end
                    end
                ...
                elseif PlayerRace == "Skypiea" then
                    -- Skypiea Trial
                    for _, part in pairs(game:GetService("Workspace").Map.SkyTrial.Model:GetDescendants()) do
                        if part.Name == "snowisland_Cylinder.081" then
                            BTPZ(part.CFrame * CFrame.new(0, 0, 0))
                        end
                    end

                elseif PlayerRace == "Fishman" then
                    -- Fishman Trial
                    for _, part in pairs(game:GetService("Workspace").SeaBeasts.SeaBeast1:GetDescendants()) do
                        if part.Name == "HumanoidRootPart" then
                            if not Pos then
                                Pos = CFrame.new(0, 30, 0)
                            end

                            Tween(part.CFrame * Pos)
                            ...
                            for _, weaponType in pairs(weaponTypes) do
                                ...
                                local keys = {122, 120, 99} -- Z, X, C
                                for _, key in pairs(keys) do
                                    game:GetService("VirtualInputManager"):SendKeyEvent(true, key, false, Player.Character.HumanoidRootPart)
                                    game:GetService("VirtualInputManager"):SendKeyEvent(false, key, false, Player.Character.HumanoidRootPart)
                                    wait(0.2)
                                end
                ...
                elseif PlayerRace == "Mink" then
                    ...
                    for _, object in pairs(game:GetService("Workspace"):GetDescendants()) do
                        if object.Name == "StartPoint" then
                            Tween(object.CFrame * CFrame.new(0, 10, 0))
                        end
                    end
                end
            end)
...
Trial:AddToggle({
    Name = "Auto Kill Players In Trial",
    Flag = "Trial/AutoKillTrial",
    Description = "Manual Use Skill To Win Only Melee Skills",
    Default = false,
    Callback = function(value)
        getgenv().AutoKillTrial = value
        spawn(function()
            while wait() do
                pcall(function()
                    if getgenv().AutoKillTrial then
                        for _, player in pairs(game:GetService("Players"):GetPlayers()) do
                            if player.Name ~= game.Players.LocalPlayer.Name and player.Character and player.Character:FindFirstChild("HumanoidRootPart") and player.Character:FindFirstChild("Humanoid") then
                                if (player.Character.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 100 and player.Character.Humanoid.Health > 0 then
                                    repeat
                                        wait(getgenv().Fast_Delay or 0.1)
                                        EquipTool(SelectWeapon)
                                        AutoHaki()
                                        ...
                                        AttackNoCoolDown()
                                    until not getgenv().AutoKillTrial or not player.Parent or player.Character.Humanoid.Health <= 0
                                end
                            end
                        end
                    end
...
Trial:AddButton({
    Name = "Viper Fly Character",
    Flag = "Trial/ViperFly",
    Description = "Use For Draco Trial And Normal Trial Ez Win",
    Callback = function()
...
Trial:AddSection("Awakening")

Trial:AddToggle({
    Name = "Auto Training",
    Flag = "Trial/AutoFarmActiveV4",
    Description = "<font color='rgb(255, 0, 0)'>Farm Bones & V4 Active But Not Stop Farming</font>",
    Default = false,
    Callback = function(v)
        _G.BoneFarmV4 = v
        _G.AutoFarmBones = v -- Enable new bone farm system
        hasTeleportedToGraveIsland = false
    end
})

-- Auto V4 Activate Handler
spawn(function()
    while task.wait() do
...
-- New Bone Farm System
spawn(function()
    local hasTeleportedToBoneIsland = false
    local currentMobIndex = 1
    local mobsCleared = {false, false, false, false}
    ...
    while task.wait() do
        if _G.AutoFarmBones then
            pcall(function()
                ...
                for _, enemy in pairs(workspace.Enemies:GetChildren()) do
                    if enemy.Name == currentMobName and enemy:FindFirstChild("Humanoid") and enemy:FindFirstChild("HumanoidRootPart") and enemy.Humanoid.Health > 0 then
                        targetMob = enemy
                        break
                    end
                end
                ...
                if targetMob then
                    ...
                    if currentMobIndex > #mobsToFarm then
                        local allCleared = true
                        for _, cleared in pairs(mobsCleared) do
                            if not cleared then
                                allCleared = false
                                break
                            end
                        ...
                        if allCleared then
                            currentMobIndex = 1
                            for i = 1, #mobsCleared do mobsCleared[i] = false end
                            task.wait(3)
                        else
                            for i = 1, #mobsCleared do
                                if not mobsCleared[i] then
...
Trial:AddToggle({
    Name = "Auto Buy Gear",
    Flag = "Trial/AutoBuyAncientQuest",
    Default = false,
    Callback = function(value)
        _G.AutoBuyAncientOne = value
    end
})

spawn(function()
    while task.wait(1) do
        if _G.AutoBuyAncientOne then
            pcall(function()
...
Visual:AddDropdown({
    ...
    Callback = function(option)
        ...
        if cam then
            cam.FieldOfView = tonumber(option)
        end
    end
})
...
Visual:AddToggle({
    ...
    Callback = function(Value)
        ...
        if Value then
            task.spawn(function()
                while getgenv().DevilFruitESP do
                    task.wait()
                    UpdateDevilChams()
                end
            end)
...
Visual:AddToggle({
    Name = "ESP Real Fruit",
    Flag = "Visual/RealFruitESP",
    Description = "",
    Default = false,
    Callback = function(Value)
        getgenv().RealFruitESP = Value
...
Raids:AddSection("Select Raid")

Raids:AddDropdown({
    Name = "Select Raid",
    Flag = "Raids/SelectChip",
    Default = "Flame", -- Pick one from the list or leave it as nil (no quotes)
    Options = {
        "Flame", "Ice", "Quake", "Light", "Dark", "Spider",
        "Rumble", "Magma", "Buddha", "Sand", "Phoenix", "Dough"
    },
    Callback = function(option)
        SelectChip = option
    end
...
spawn(function()
    while wait(1) do -- Slower check to reduce lag
        if _G.Auto_StartRaid then
            pcall(function()
                ...
                -- Check for chip in both backpack and character
                local hasChip = backpack:FindFirstChild("Special Microchip") or (char and char:FindFirstChild("Special Microchip"))
                
                -- Check if raid is already in progress
                local timerVisible = player.PlayerGui.Main.Timer.Visible
                local hasIsland = workspace["_WorldOrigin"].Locations:FindFirstChild("Island 1")
                
                if not timerVisible and not hasIsland then
                    if hasChip then
                        -- Set spawn point first
                        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("SetSpawnPoint")
                        
                        -- Different handling based on sea level
                        if Sea2 then
                            -- Teleport to Second Sea raid location
                            Tween2(CFrame.new(-6438.73535, 250.645355, -4501.50684))
                            wait(1) -- Wait for teleport to complete
                            
                            -- Make sure we're close to the button
                            local button = workspace.Map.CircleIsland.RaidSummon2.Button
                            if button and button:FindFirstChild("Main") and button.Main:FindFirstChild("ClickDetector") then
                                local distance = (button.Main.Position - player.Character.HumanoidRootPart.Position).Magnitude
                                if distance <= 10 then
                                    fireclickdetector(button.Main.ClickDetector)
                                    print("Clicked Second Sea raid button")
                                else
                                    -- Try to teleport closer
                                    Tween2(button.Main.CFrame)
                                    wait(1)
                                    fireclickdetector(button.Main.ClickDetector)
                                    print("Clicked Second Sea raid button after repositioning")
                                end
                            end
                        elseif Sea3 then
                            -- Request entrance to Third Sea raid location
                            ...
                            -- Make sure we're close to the button
                            local button = workspace.Map["Boat Castle"].RaidSummon2.Button
                            if button and button:FindFirstChild("Main") and button.Main:FindFirstChild("ClickDetector") then
                                local distance = (button.Main.Position - player.Character.HumanoidRootPart.Position).Magnitude
                                if distance <= 10 then
                                    fireclickdetector(button.Main.ClickDetector)
                                    print("Clicked Third Sea raid button")
                                else
                                    -- Try to teleport closer
                                    Tween2(button.Main.CFrame)
                                    wait(1)
                                    fireclickdetector(button.Main.ClickDetector)
                                    print("Clicked Third Sea raid button after repositioning")
                                end
                            end
                        end
                    else
                        print("No Special Microchip found!")
                    end
                end
            end)
        end
...
Raids:AddToggle({
    Name = "Instant Start Raid",
    Flag = "Raids/AutoRaidSummon",
    Callback = function(value)
        getgenv().AutoRaidSummon = value
        if value then
            spawn(function()
                while getgenv().AutoRaidSummon do
                    pcall(function()
                        if World2 then
                    ...
                    task.wait(1) -- Add delay to prevent crash
                end
            end)
        end
    end
        ...
        if _G.AutoRaid then
            pcall(function()
                ...
                for _, enemy in pairs(workspace.Enemies:GetChildren()) do
                    if enemy:FindFirstChild("Humanoid") and enemy:FindFirstChild("HumanoidRootPart") and enemy.Humanoid.Health > 0 then
                        local dist = (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - enemy.HumanoidRootPart.Position).Magnitude
                        if dist < ClosestDistance and dist <= 1000 then
                            Closest = enemy
                            ClosestDistance = dist
                        end
                    end
                ...
                if Closest then
                    repeat
                        ...
                        AutoHaki()
                        EquipTool(SelectWeapon)
                        Tween(CFrame.new(Closest.HumanoidRootPart.Position + Vector3.new(0, 50, 0)))
                        AttackNoCoolDown()
                        ...
                        bringmob = true
                    until not _G.AutoRaid or Closest.Humanoid.Health <= 0 or not Closest.Parent
                    bringmob = false
                else
                    local NextIsland = getNextIsland()
                    if NextIsland then
                        Tween(NextIsland.CFrame)
                    end
                end
            end)
        end
    ...
    while task.wait(1) do
        if AutoNextIsland then
            pcall(function()
                ...
                -- Reset visited if at start of raid
                if (pos - Vector3.new(-6438.735, 250.645, -4501.506)).Magnitude < 10 or
                   (pos - Vector3.new(-5017.408, 314.844, -2823.012)).Magnitude < 10 then
                    VisitedIslands = {}
                end
                ...
                for i = 2, 5 do
                    ...
                    if island and not VisitedIslands[islandName] then
                        ...
                        wait(5) -- give time to arrive before going to next
                        break -- wait before going to next island
                    end
                end
            end)
        end
...
Raids:AddSection("Fruits");

Raids:AddToggle({
    Name = "Auto Spin Random Fruit",
    Flag = "Raids/EnableRandomFruit", -- Unique flag for the toggle
    Description = "",
    Default = false, -- Default state of the toggle (false = off)
    Callback = function(Value)
        if Value then
            -- If toggle is enabled (True), execute the random fruit purchase
...
-- Toggle for Teleport Collect Fruit under Raid tab
Raids:AddToggle({
    Name = "Teleport Collect Fruit",
    Flag = "Raids/TeleportCollectFruit",
                ...
                if char and char:FindFirstChild("HumanoidRootPart") then
                    for _, obj in pairs(workspace:GetChildren()) do
                        if obj:IsA("Tool") and string.find(obj.Name, "Fruit") and obj:FindFirstChild("Handle") then
                            -- Call your custom Tween function here
                            Tween(obj.Handle.CFrame)  -- Using your existing custom Tween
                            task.wait(0.1)
                            break
                        end
                    end
...
Raids:AddToggle({
    Name = "Instant Teleport To Fruit",
    Flag = "Raids/TeleportCollectFruit",
    Description = "Instantly collect fruits if spawned or dropped.",
    Default = false,
    Callback = function(Value)
...
local IslandCFrames = {
    ...
    ["Marine Ford"] = CFrame.new(-4914.82, 50.96, 4281.02),
...
-- RequestEntrance Islands
local function TeleportSpecialIsland(name)
    if name == "Sky Island 2" then
        CommF_:InvokeServer("requestEntrance", Vector3.new(-4607.82, 872.54, -1667.55))
    elseif name == "Sky Island 3" then
        CommF_:InvokeServer("requestEntrance", Vector3.new(-7894.61, 5547.14, -380.29))
    elseif name == "Underwater Island" then
        CommF_:InvokeServer("requestEntrance", Vector3.new(61163.85, 11.67, 1819.78))
    end
end

-- Dropdown
Teleport:AddDropdown({
    Name = "Select Island Teleport",
    Flag = "Teleport/IslandSelection",
    Default = "WindMill",
    Options = {
        "WindMill", "Marine", "Middle Town", "Jungle", "Pirate Village", "Desert", "Snow Island",
        "Marine Ford", "Colosseum", "Sky Island 1", "Sky Island 2", "Sky Island 3",
        "Prison", "Magma Village", "Underwater Island", "Fountain City"
    },
    Callback = function(option)
        _G.SelectedTeleportIsland = option
        print("Selected Island:", _G.SelectedTeleportIsland)
    end
})
...
Teleport:AddToggle({
    ...
    Flag = "Teleport/EnableTeleport",
    Default = false,
    Callback = function(Value)
        if Value and game.PlaceId == 2753915549 then -- First Sea
            if IslandCFrames[_G.SelectedTeleportIsland] then
                Tween(IslandCFrames[_G.SelectedTeleportIsland])
            else
                TeleportSpecialIsland(_G.SelectedTeleportIsland)
            end
        end
    end
})
...
local IslandCFramesSea2 = {
    ...
    ["Kingdom Of Rose"] = CFrame.new(-11.311455726624, 29.276733398438, 2771.5224609375),
    ...
    ["Graveyard Island"] = CFrame.new(-5622.033203125, 492.19604492188, -781.78552246094),
    ["Snow Mountain"] = CFrame.new(753.14288330078, 408.23559570313, -5274.6147460938),
    ["Hot & Cold"] = CFrame.new(-6127.654296875, 15.951762199402, -5040.2861328125),
...
local function TeleportSpecialIslandSea2(name)
    if name == "The Cafe" or name == "Flamingo Mansion" then
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(-281.93707275390625, 306.130615234375, 609.280029296875))
        wait()
        if name == "The Cafe" then
            Tween2(CFrame.new(-380.47927856445, 77.220390319824, 255.82550048828))
        end
    elseif name == "Flamingo Room" then
    ...
    elseif name == "Cursed Ship" then
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(923.40197753906, 125.05712890625, 32885.875))
    end
end


Teleport:AddSection("Sea 2")

Teleport:AddDropdown({
    Name = "Select Island Teleport",
    Flag = "Teleport/IslandSelectionSea2",
    Default = "The Cafe",
    Options = {
        "The Cafe", "Shank Room", "Mob Island", "Kingdom Of Rose", "Flamingo Mansion", "Flamingo Room", "Green Zone",
        "Factory", "Colossuim", "Graveyard Island", "Snow Mountain", "Hot & Cold", "Cursed Ship", "Ice Castle", "Forgotten Island"
    },
    Callback = function(option)
        _G.SelectedTeleportIslandSea2 = option
        print("Selected Sea 2 Island:", _G.SelectedTeleportIslandSea2)
    end
})
...
Teleport:AddToggle({
    Name = "Teleport To Island",
    Flag = "Teleport/EnableTeleportSea2",
    Default = false,
    Callback = function(Value)
        if Value and game.PlaceId == 4442272183 then -- Second Sea
            if IslandCFramesSea2[_G.SelectedTeleportIslandSea2] then
                Tween(IslandCFramesSea2[_G.SelectedTeleportIslandSea2])
            else
                TeleportSpecialIslandSea2(_G.SelectedTeleportIslandSea2)
            end
        end
    end
})
...
local function TeleportSpecialIslandSea3(name)
...
Teleport:AddSection("Sea 3")

Teleport:AddDropdown({
    Name = "Select Island Teleport",
    Flag = "Teleport/IslandSelectionSea3",
    Default = "Port Town",
    Options = {
        "Ussop Island", "Mini Sky Island", "Great Tree", "Castle On The Sea",
        "MiniSky", "Port Town", "Hydra Island", "Floating Turtle", "Mansion",
        "Haunted Castle", "Ice Cream Island", "Peanut Island", "Cake Island",
        "Cocoa Island", "Candy Island", "Tiki Outpost"
    },
    Callback = function(option)
        _G.SelectedTeleportIslandSea3 = option
        print("Selected Sea 3 Island:", _G.SelectedTeleportIslandSea3)
    end
})

Teleport:AddToggle({
    Name = "Teleport To Island",
    Flag = "Teleport/EnableTeleportSea3",
    Default = false,
    Callback = function(Value)
        if Value and game.PlaceId == 7449423635 then -- Sea 3 ID
            local selected = _G.SelectedTeleportIslandSea3
            if IslandCFramesSea3[selected] then
                Tween(IslandCFramesSea3[selected])
            else
                TeleportSpecialIslandSea3(selected)
            end
        end
    end
})
...
Teleport:AddSection("Sea World");

Teleport:AddToggle({
    Name = "Teleport To Sea 1",
    Flag = "Teleport/ToSea1",
    Default = false,
    Callback = function(Value)
        if Value then
            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("TravelMain")
            task.wait(0.2)
            getgenv().Flags["Teleport/ToSea1"] = false
        end
    end
})
...
Teleport:AddToggle({
    ...
    Flag = "Teleport/ToSea2",
    Default = false,
    Callback = function(Value)
        if Value then
            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("TravelDressrosa")
            task.wait(0.2)
            getgenv().Flags["Teleport/ToSea2"] = false
        end
    end
})
...
Teleport:AddToggle({
    ...
    Flag = "Teleport/ToSea3",
    Default = false,
    Callback = function(Value)
        if Value then
            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("TravelZou")
            task.wait(0.2)
            getgenv().Flags["Teleport/ToSea3"] = false
        end
    end
})
...
Sea:AddToggle({
    ...
    Callback = function(Value)
        ...
        if getgenv().WKeyPressLoop then
            getgenv().WKeyPressLoop:Disconnect()
            getgenv().WKeyPressLoop = nil
        end
        
        -- Reset boat properties when toggled off
        if not Value then
            local Players = game:GetService("Players")
            local LocalPlayer = Players.LocalPlayer
            local Character = LocalPlayer.Character
            
            -- Find the boat the player is in
            for _, boat in pairs(workspace.Boats:GetChildren()) do
                local seat = boat:FindFirstChild("VehicleSeat")
                if seat and seat.Occupant and Character and seat.Occupant.Parent == Character then
                    -- Re-enable collision
        ...
        -- Create separate loop for W key pressing
        getgenv().WKeyPressLoop = game:GetService("RunService").Heartbeat:Connect(function()
            if getgenv().AutoDriveBoat then
                -- Use virtual input object to simulate W key press
                game:GetService("VirtualInputManager"):SendKeyEvent(true, Enum.KeyCode.W, false, game)
                task.wait(0.1) -- Small delay to avoid spamming input
            end
        end)
        
        -- Main loop for boat control and collision
        getgenv().AutoDriveBoatConnection = game:GetService("RunService").Heartbeat:Connect(function()
            ...
            for _, boat in pairs(workspace.Boats:GetChildren()) do
                ...
                if seat and seat.Occupant and seat.Occupant == Humanoid then
                    ...
                    -- Disable collision for boat parts
                    for _, part in pairs(boat:GetDescendants()) do
                        if part:IsA("BasePart") then
                            part.CanCollide = false
                        end
...
local BoatSeats = {}
local selectedBoat = "Beast Hunter"



-- Toggle for Flying to Boat Dealer at Tiki Outpost
Sea:AddToggle({
    Name = "Fly To Boat Dealer Tiki Outpost",
    Flag = "Sea/FlyToBoatDealerTiki",
    Description = "",
    Default = false,
    Callback = function(Value)
        _G.FlyToBoatDealer = Value
...
Sea:AddDropdown({
    ...
    Options = {
        ...
        "Lantern", "Dinghy", "PirateSloop", "PirateBrigade",
        "PirateGrandBrigade", "MarineGrandBrigade", "MarineBrigade", "MarineSloop"
    },
    Callback = function(option)
        selectedBoat = option
    end
...
-- Toggle for Buying Selected Boat
Sea:AddToggle({
    Name = "Buy Selected Boat",
    Flag = "Sea/BuyBoat",
    Description = "Toggle to buy the boat selected from the dropdown",
    Default = false,
    Callback = function(Value)
        if Value then
...
-- Toggle for Flying to the Selected Boat
Sea:AddToggle({
    Name = "Fly To Boat",
    Flag = "Sea/FlyToBoat",
    Description = "Toggle to fly to the boat you bought",
    Default = false,
    Callback = function(Value)
        _G.FlyToBoatActive = Value
...
function BuyBoat(boatName)
    ...
    -- Clear previous boat entries
    BoatSeats = {}
    
    -- Give time for the boat to spawn
    task.wait(3)
    
    -- Search for the boat in workspace
    RefreshBoats()
end

function RefreshBoats()
    BoatSeats = {}
    for _, model in pairs(workspace.Boats:GetChildren()) do
        if model:IsA("Model") then
            local seat = model:FindFirstChild("VehicleSeat")
            if seat and not seat.Occupant then
                print("Found boat: " .. model.Name)
                BoatSeats[model.Name] = seat
            end
        end
    end
end

function FlyToBoatSeat()
    if _G.FlyToBoatActive then
        print("Attempting to fly to boat seat")
        -- First refresh to make sure we have the latest boats
        RefreshBoats()
        
        -- Try to find the selected boat first
        if selectedBoat and BoatSeats[selectedBoat] then
            local seat = BoatSeats[selectedBoat]
            if seat and seat.Parent and not seat.Occupant then
                print("Flying to selected boat: " .. selectedBoat)
                Tween2(seat.CFrame)
                return
            end
        end
        
        -- If selected boat not found, try any available boat
        for boatName, seat in pairs(BoatSeats) do
            if seat and seat.Parent and not seat.Occupant then
                print("Flying to available boat: " .. boatName)
                Tween2(seat.CFrame)
                return
            end
        end
        
        print("No available boat seats found")
    end
end

-- Create a refresh loop for available boats
spawn(function()
    while wait(3) do
        RefreshBoats()
    end
    ...
    while wait() do
        if _G.AutoTerrorshark then
            pcall(function()
                ...
                local healthRecoveryStartTime = 0

                if humanoid then
                    if humanoid.Health < (humanoid.MaxHealth * 0.3) then
                        lowHealth = true
                        healthRecoveryStartTime = tick()
                    end
                end
                
                if game:GetService("Workspace").Enemies:FindFirstChild("Terrorshark") then
                    for _, v1445 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        if v1445.Name == "Terrorshark" and v1445:FindFirstChild("Humanoid") and v1445:FindFirstChild("HumanoidRootPart") and v1445.Humanoid.Health > 0 then
                            local flyingUp = false
                            local lastMoveTime = 0
                            local attackDetected = false
                            local normalHeight = 50 -- Normal positioning at 50 studs
                            
                            local function isTerrosharkAttacking()
                                if v1445:FindFirstChild("Humanoid") and v1445.Humanoid:FindFirstChild("Animator") then
                                    local animator = v1445.Humanoid.Animator
                                    local playingTracks = animator:GetPlayingAnimationTracks()
                                    
                                    for _, track in pairs(playingTracks) do
                                        local animName = track.Animation.Name:lower()
                                        if animName:find("attack") or animName:find("skill") or animName:find("special") then
                                            return true
                                        end
                                    end
                                end
                                ...
                                for _, child in pairs(workspace:GetChildren()) do
                                    if child.Name:find("TerrorsharkProjectile") or child.Name:find("TerrorsharkAttack") then
                                        return true
                                    end
                                end
                            ...
                            repeat
                                ...
                                if humanoid then
                                    if lowHealth then
                                        if humanoid.Health >= 5000 or (tick() - healthRecoveryStartTime > 15) then
                                            lowHealth = false
                                        else
                                            if v1445 and v1445:FindFirstChild("HumanoidRootPart") then
                                                Tween(v1445.HumanoidRootPart.CFrame * CFrame.new(0, 180, 0))
                                            end
                                            wait(1)
                                            continue
                                        end
                                ...
                                if v1445 and v1445:FindFirstChild("HumanoidRootPart") and v1445:FindFirstChild("Humanoid") then
                                    v1445.HumanoidRootPart.CanCollide = false
                                    v1445.Humanoid.WalkSpeed = 0
                                    v1445.HumanoidRootPart.Size = Vector3.new(100, 100, 100)
                                    
                                    attackDetected = isTerrosharkAttacking()
                                    
                                    local currentTime = tick()
                                    
                                    if attackDetected and not flyingUp then
                                        flyingUp = true
                                        Tween(v1445.HumanoidRootPart.CFrame * CFrame.new(0, 170, 0))
                                        wait(1)
                                        flyingUp = false
                                        attackDetected = false
                                        
                                        if v1445 and v1445:FindFirstChild("HumanoidRootPart") then
                                            Tween(v1445.HumanoidRootPart.CFrame * CFrame.new(0, normalHeight, 0))
                                        end
                                    elseif currentTime - lastMoveTime >= 3 and not flyingUp and not attackDetected then
                                        flyingUp = true
                                        Tween(v1445.HumanoidRootPart.CFrame * CFrame.new(0, 160, 0))
                                        wait(0.5)
                                        flyingUp = false
                                    ...
                                    elseif not flyingUp and not attackDetected then
                                        Tween(v1445.HumanoidRootPart.CFrame * CFrame.new(0, normalHeight, 0))
                                    end
                                end
                            until not _G.AutoTerrorshark or not v1445 or not v1445.Parent or not v1445:FindFirstChild("Humanoid") or v1445.Humanoid.Health <= 0
                        end
                    end
                elseif game:GetService("ReplicatedStorage"):FindFirstChild("Terrorshark") then
                    Tween(game:GetService("ReplicatedStorage"):FindFirstChild("Terrorshark").HumanoidRootPart.CFrame * CFrame.new(0, 50, 0))
                end
            end)
        end
    end
...
Sea:AddToggle({
    ...
    Callback = function(v)
        ...
        game:GetService("RunService").RenderStepped:Connect(function()
            ...
            local function EnterEmptyBoat()
                if Busy then return end
                Busy = true

                for _, seat in pairs(BoatsCache) do
            ...
            if not inBoat then return end

            currentSeat.MaxSpeed = v508  -- This is where boat speed connects
            currentSeat.CFrame = CFrame.new(currentSeat.Position) * currentSeat.CFrame.Rotation

            game:GetService("VirtualInputManager"):SendKeyEvent(true, "W", false, game)

            for _, part in pairs(game.Workspace.Boats:GetDescendants()) do
            ...
            for _, name in ipairs(islandsToDelete) do
                local island = game.Workspace.Map:FindFirstChild(name)
                if island and island:IsA("Model") then
                    island:Destroy()
                end
            end

            local mirage = game.Workspace.Map:FindFirstChild("MysticIsland")
            if mirage then
...
Sea:AddToggle({
    ...
    Callback = function(v)
        ...
        game:GetService("RunService").RenderStepped:Connect(function()
            ...
            local prehistoric = game.Workspace.Map:FindFirstChild("PrehistoricIsland")
            if prehistoric then
                game:GetService("VirtualInputManager"):SendKeyEvent(false, "W", false, game)
...
Sea:AddToggle({
    ...
    Callback = function(v)
        ...
        game:GetService("RunService").RenderStepped:Connect(function()
            ...
            local frozenDimension = game.Workspace.Map:FindFirstChild("FrozenDimension")
...
Sea:AddToggle({
    Name = "Auto Teleport To Tiki Outpost",
...
game:GetService("RunService").RenderStepped:Connect(function()
    ...
    if seat then
        seat.MaxSpeed = v508  -- Boat speed is connected to the slider

        local tikiPosition = CFrame.new(-16217.7568359375, 9.126761436462402, 446.06536865234375)
        local boatPosition = seat.Position
        local targetPosition = tikiPosition.Position
        local direction = (targetPosition - boatPosition).unit
        local speed = seat.MaxSpeed * game:GetService("RunService").RenderStepped:Wait()

        -- Move the boat towards Tiki Outpost
        seat.CFrame = seat.CFrame + direction * speed

        -- Rotate the boat towards the target position
        seat.CFrame = CFrame.new(seat.Position, targetPosition)

        -- Check if the boat has reached Tiki Outpost
        if (seat.Position - targetPosition).magnitude < 120 then
            _G.AutoComeTiki = false
            game:GetService("VirtualInputManager"):SendKeyEvent(false, "W", false, game)  -- Stop boat
        end
    end
end)

Sea:AddToggle({
    Name = "Auto Teleport To Hydra Island",
    Description = "Use Boat",
    Default = false,
    Callback = function(v)
        _G.AutoComeHydra = v
...
game:GetService("RunService").RenderStepped:Connect(function()
    ...
    -- Find the boat the player is on
    for _, boat in pairs(game.Workspace.Boats:GetChildren()) do
        local vehicleSeat = boat:FindFirstChild("VehicleSeat")
        if vehicleSeat and vehicleSeat.Occupant == humanoid then
            seat = vehicleSeat
    ...
    if seat then
        ...
        local hydraPosition = CFrame.new(5193.9375, -0.04690289497375488, 1631.578369140625)
        local boatPosition = seat.Position
        local targetPosition = hydraPosition.Position
        local direction = (targetPosition - boatPosition).unit
        local speed = seat.MaxSpeed * game:GetService("RunService").RenderStepped:Wait()

        -- Move the boat towards Hydra Island
        seat.CFrame = seat.CFrame + direction * speed

        -- Rotate the boat towards the target position
        seat.CFrame = CFrame.new(seat.Position, targetPosition)

        -- Check if the boat has reached Hydra Island
        if (seat.Position - targetPosition).magnitude < 120 then
...
v23.Image = "rbxassetid://101883839381557"
getgenv().LogoButton = v23  -- Fixed image ID (removed extra digits)
...
elseif ((v197 == 2575) or (v197 <= 2649) or (SelectMonster == "Skull Slayer") or (v197 == 2650)) then
...
-- Tween logic (replaces Tween and Tween2)
local connection, tween, isTweening = nil, nil, false
TweenSpeed = 350
