...
Setting:AddToggle({
    Name = "Super Bring Mobs",
    Flag = "Setting/SuperBringMobs",
    Description = "",
    Callback = function(value)
        getgenv().SuperBringMob = value
        if value then
            spawn(function()
                while getgenv().SuperBringMob do
                    pcall(function()
                        local mobsToFarm = {}
                        local mobCount = 0
                        local maxMobs = 5  -- Maximum number of mobs to bring at once
                        
                        -- First pass: identify mobs to bring
                        for _, mob in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if mob.Name == MonFarm and mob:FindFirstChild("Humanoid") and mob:FindFirstChild("HumanoidRootPart") and mob.Humanoid.Health > 0 then
                                ...
                                if root and FarmPos and (root.Position - FarmPos.Position).Magnitude <= 1000 then
                                    table.insert(mobsToFarm, mob)
                                    mobCount = mobCount + 1
                                    
                                    if mobCount >= maxMobs then
                                        break
                                    end
                                end
                            end
                        ...
                        -- Second pass: process identified mobs
                        for i, mob in ipairs(mobsToFarm) do
                            local root = mob:FindFirstChild("HumanoidRootPart")
                            if root then
                                -- Create offset positions around FarmPos to avoid stacking
                                local offset
                                if i == 1 then
                                    offset = Vector3.new(0, 0, 0)  -- Center mob
                                elseif i == 2 then
                                    offset = Vector3.new(10, 0, 0)  -- Right
                                elseif i == 3 then
                                    offset = Vector3.new(-10, 0, 0)  -- Left
                                elseif i == 4 then
                                    offset = Vector3.new(0, 0, 10)  -- Front
                                else
                                    offset = Vector3.new(0, 0, -10)  -- Back
                                end
                                
                                -- Apply position with offset
                                root.CFrame = FarmPos * CFrame.new(offset)
                                ...
                                -- Disable mob movement and animations
                                ...
                                -- Force mob to stay in place
                                mob.Humanoid:ChangeState(11)  -- Seated state
                                mob.Humanoid:ChangeState(14)  -- Physics state
                            end
                        end
                        
                        -- Increase simulation radius to maintain control over distant mobs
                        sethiddenproperty(game.Players.LocalPlayer, "SimulationRadius", math.huge)
                        sethiddenproperty(game.Players.LocalPlayer, "MaximumSimulationRadius", math.huge)
                    end)
                    wait(0.1)  -- Small wait to avoid performance issues
                end
            end)
        end
    end
